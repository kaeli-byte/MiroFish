...
def create_engine_simulation():
    """Create multi-engine simulation (default OASIS, supports mesa_renewable_fuels)."""
    data = request.get_json() or {}

    engine = data.get('engine', 'oasis')
    if engine not in ENGINE_REGISTRY.list_engines():
        return jsonify({"success": False, "error": f"Unsupported engine: {engine}"}), 400
...
def prepare_engine_simulation(simulation_id: str):
    """Unified prepare endpoint; renewables scenario uses LLM for assumptions only in this phase."""
    try:
...
def get_engine_simulation_results(simulation_id: str):
    """Unified results endpoint (JSON for custom charts)."""
    try:
        engine, _ = _resolve_engine(simulation_id)
